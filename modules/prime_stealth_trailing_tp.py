...
class StealthConfig:
    ...
    # Breakeven protection (OPTIMIZED FOR MAXIMUM PROFIT CAPTURE)
    breakeven_threshold_pct: float = 0.005  # 0.5% activation threshold for better profit protection
    breakeven_offset_pct: float = 0.002     # 0.2% offset above breakeven for maximum profit protection
    
    # Trailing stop parameters (OPTIMIZED FOR MAXIMUM PROFIT CAPTURE)
    base_trailing_pct: float = 0.008        # 0.8% base trailing distance for aggressive profit capture
    min_trailing_pct: float = 0.004         # 0.4% minimum trailing for tighter control
    max_trailing_pct: float = 0.06          # 6% maximum trailing for better risk management
    ...
    # Take profit targets (OPTIMIZED FOR BIGGER GAINS)
    base_take_profit_pct: float = 0.05      # 5% base take profit (vs 2% before)
    trending_take_profit_pct: float = 0.12  # 12% trending moves
    explosive_take_profit_pct: float = 0.25 # 25% explosive moves (vs 10% before)
    moon_take_profit_pct: float = 0.50      # 50% moon moves (vs 25% before)
...
class PrimeStealthTrailingTP:
    ...
    async def update_position(self, symbol: str, market_data: Dict[str, Any]) -> Optional[StealthDecision]:
        ...
        try:
            ...
            # Store RSI for decision making (add to position state if not already there)
            if not hasattr(position_state, 'current_rsi'):
    ...
    def _check_exit_conditions(self, position: PositionState, market_data: Dict[str, Any]) -> Optional[StealthDecision]:
        """Check for immediate exit conditions - PRIORITY: Automatic position closure when price falls below stealth stop"""
        current_price = position.current_price
        
        # PRIORITY 1: STEALTH STOP LOSS HIT - AUTOMATIC POSITION CLOSURE
        # This is the most critical check - if price falls below our stealth stop, close immediately
        if current_price <= position.current_stop_loss:
            log.warning(f"🚨 STEALTH STOP TRIGGERED: {position.symbol} @ ${current_price:.2f} <= ${position.current_stop_loss:.2f}")
            log.warning(f"🚨 TRIGGERING AUTOMATIC E*TRADE SELL ORDER FOR POSITION CLOSURE")
            return StealthDecision(
                action="EXIT",
                exit_reason=ExitReason.STOP_LOSS,
                reasoning=f"STEALTH STOP HIT: ${current_price:.2f} <= ${position.current_stop_loss:.2f} - AUTOMATIC E*TRADE CLOSURE"
            )
        
        # PRIORITY 2: TAKE PROFIT HIT
        if current_price >= position.take_profit:
            log.info(f"💰 TAKE PROFIT TRIGGERED: {position.symbol} @ ${current_price:.2f} >= ${position.take_profit:.2f}")
            return StealthDecision(
                action="EXIT",
        ...
        # PRIORITY 3: RSI-based exit for losing positions (momentum loss protection)
        current_rsi = getattr(position, 'current_rsi', market_data.get('rsi', 50.0))
        if current_rsi < 45 and position.unrealized_pnl_pct < 0:
            log.warning(f"📉 RSI EXIT: {position.symbol} RSI={current_rsi:.1f} with losing position")
            return StealthDecision(
                action="EXIT",
                exit_reason=ExitReason.MOMENTUM_EXIT,
                reasoning=f"RSI below 45 ({current_rsi:.1f}) with losing position"
            )
        
        # PRIORITY 4: Time-based exit (maximum holding period)
        holding_hours = (datetime.utcnow() - position.entry_time).total_seconds() / 3600
        if holding_hours >= self.config.max_holding_hours:
            log.info(f"⏰ TIME EXIT: {position.symbol} maximum holding time reached ({holding_hours:.1f} hours)")
            return StealthDecision(
                action="EXIT",
