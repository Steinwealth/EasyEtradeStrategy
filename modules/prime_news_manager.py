...
class PrimeNewsManager:
    ...
    async def analyze_news_sentiment(self, symbol: str, lookback_hours: int = None) -> NewsSentimentResult:
        """
        Comprehensive news sentiment analysis for a symbol
        """
        ...
        # Polygon news
        polygon_news = await self._fetch_polygon_news(symbol, from_time)
        all_news_items.extend(polygon_news)
        
        # Finnhub news
        finnhub_news = await self._fetch_finnhub_news(symbol, from_time)
