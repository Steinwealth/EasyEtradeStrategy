...
class BuyersVolumeSurgingStrategy(BaseStrategy):
    ...
    def __init__(self):
        ...
        self.min_rsi = 55.0
        self.min_volume_ratio = 1.2  # 20% above average volume
    ...
    async def analyze(self, symbol: str, market_data: Dict[str, Any]) -> StrategyResult:
        try:
            ...
            if len(prices) < 15 or not volumes:
                return StrategyResult(
                    strategy_type=self.strategy_type,
                    symbol=symbol,
                    should_trade=False,
            ...
            current_volume = volumes[-1]
            avg_volume = np.mean(volumes[-20:]) if len(volumes) >= 20 else volumes[0]
            volume_ratio = current_volume / avg_volume if avg_volume > 0 else 1.0
            
            # Determine if should trade (RSI > 55 + positive volume surging)
            should_trade = rsi > self.min_rsi and volume_ratio > self.min_volume_ratio
            
            # Calculate confidence
            rsi_confidence = min(1.0, (rsi - 50) / 30)  # Scale RSI 50-80 to 0-1
            volume_confidence = min(1.0, (volume_ratio - 1.0) / 2.0)  # Scale volume ratio
            confidence = (rsi_confidence + volume_confidence) / 2
            
            # Calculate position size
            position_size_pct = min(4.0, confidence * 4.0)
            ...
            return StrategyResult(
                ...
                take_profit=take_profit,
                position_size_pct=position_size_pct,
                reasoning=f"Buyers Volume Surging: RSI {rsi:.1f}, Volume {volume_ratio:.1f}x",
                metadata={
                    'rsi': rsi,
...
class NewsSentimentStrategy(BaseStrategy):
    """News Sentiment Strategy - VADER sentiment analysis integration"""
    
    def __init__(self, news_manager=None):
        super().__init__(StrategyType.NEWS_SENTIMENT)
        self.min_sentiment_score = 0.3  # 30% positive sentiment threshold
        self.min_confidence = 0.5  # 50% confidence threshold
    ...
    async def analyze(self, symbol: str, market_data: Dict[str, Any]) -> StrategyResult:
        try:
            ...
            should_trade = abs(sentiment_score) >= self.min_sentiment_score
            
            # Calculate confidence based on sentiment strength and news quality
            confidence = min(1.0, abs(sentiment_score) * sentiment_result.sentiment_confidence * sentiment_result.news_quality_score)
            ...
            if should_trade:
                if sentiment_score > 0:
                    reasoning = f"Positive sentiment ({sentiment_score:.2f}) with {sentiment_result.news_count} news items"
                else:
                    reasoning = f"Negative sentiment ({sentiment_score:.2f}) with {sentiment_result.news_count} news items"
            else:
                reasoning = f"Neutral sentiment ({sentiment_score:.2f}) below threshold ({self.min_sentiment_score})"
            
            # Add news sentiment to market data for risk manager
            market_data['news_sentiment'] = {
                'sentiment_score': sentiment_score,
                'direction': 'positive' if sentiment_score > 0.1 else 'negative' if sentiment_score < -0.1 else 'neutral',
