...
# Add ETradeOAuth modules to path
etrade_oauth_modules_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'ETradeOAuth', 'modules')
sys.path.insert(0, etrade_oauth_modules_path)
...
class PrimeETradeTrading:
    ...
    def _load_credentials_from_secret_manager(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            log.warning(f"Could not load credentials from Secret Manager: {e}")
            return None
    ...
    def _load_tokens_from_secret_manager(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            etrade_oauth_dir = Path(__file__).parent.parent / "ETradeOAuth" / "login"
            sys.path.insert(0, str(etrade_oauth_dir))
            
            from secret_manager_oauth import get_secret_manager_oauth
            
            # Get Secret Manager instance
            secret_manager = get_secret_manager_oauth()
            
            if not secret_manager.client:
                log.warning("Secret Manager client not available for token loading")
                return None
            
            # Load tokens from Secret Manager
            tokens = secret_manager.load_tokens(self.environment)
            ...
            log.warning(f"Could not load tokens from Secret Manager: {e}")
            return None
